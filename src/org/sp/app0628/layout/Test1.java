...
public class Test1 {
	...
	public static void main(String[] args) {
		...
		//패널은 현실의 판자를 표현것이며, 색상이 없다.
		Panel[] p=new Panel[5];
		for(int i=0;i<p.length;i++) {
			p[i] = new Panel();//배열의 각요소에 패널을 생성하여 대입
		}
		...
		//포함할 능력이 있게된다...또한 다른 컴포넌트를 포함할 때는 반드시 어떤 방식으로
		//컴포넌트들을 배치할지를 결정해야 하며, 이때 사용되는 객체가 바로 배치관리자(FlowLayout, BorderLayout...)이다..
		//Frame, Panel 이건 즉  Container 형인 경우, 만일 개발자가 배치를 명시하지 않으면, 배치가 적용되지
